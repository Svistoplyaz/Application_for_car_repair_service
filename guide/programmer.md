...
Сущность отвечает за хранение информации о запасных частях, имеющихся в наличии в автомастерской. Возможно задание названия запасной части, единицы измерения (элемент перечисления `Unit`), количества запасных частей, флага универсальности детали.
...
При резервировании и списании деталей их фактическое значение, хранимое в сущности, не уменьшается. Для получения реального количества имеющихся деталей необходимо воспользоваться одним из следующих методов:
...
- `centerColumn(JTable, int)` - установка определенной колонки
- `getStart()` - получение даты начала периода.
- `getFinish()` - получение даты окончания периода.
...
Также особенности реализации имеются у страницы управления заказами - дополнительно имеется кнопка пополнения количества запасной части на складе, открывающая окно `me.svistoplyas.teamdev.graphics.otherFrames.SpareRefill`, содержащее поле ввода количества, информацию о единицах измерения, и реализующее логику пополнения количества запасных частей.
...
В форме добавления и редактирования запасной части метод `fillFields` дополнительно формирует выпадающий список с единицами измерения. В качестве дополнительных валидаций проверяется корректность ввода количества в зависимости от выбранных единиц измерения, а также выполняется проверка на записи с дублирующимся названием. При редактировании записи поле ввода количества недоступно, его изменение производится через отдельную форму, описанную ранее. В зависимости от положения флага универсальности запрещается или разрешается редактирование списка совместимых моделей.
